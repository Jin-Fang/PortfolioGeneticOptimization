...
if __name__ == "__main__":
    ...
    df = pd.read_csv('CovarianceMatrix_20X240.csv', header=None)
    c = np.array(df)

    # load return data
    df = pd.read_csv('SinglePeriod_20X12.csv', header=None)
    r = np.array(df)

    # define population related parameters
    levels = 127
    total_periods = 12
    asset_length = [18, 12, 36, 36, 12, 36, 36, 12, 36, 36, 36, 36, 24, 36, 36, 36, 24, 18, 36, 36]
    asset_num = len(asset_length)
    ...
    final_result = max(result_fitness)

    end = time.time()


    print("Final result: " + str(final_result)[1:])
    print("Final solution: " + "\n" + str(new_population[result_fitness.index(final_result)]))
    runtime = end - start
    print("Run time: ")
    print(runtime)
    ...
    # =============================================
    # port_weights = generate_chromosome(levels, total_periods, asset_length)
    #
    # #load covariance matrix
    # df = pd.read_csv('CovarianceMatrix_20X240.csv', header=None)
    # c = np.array(df)
    # print(c)
    #
    # # calculate variance of a portfolio example
    # # specific_weights = port_weights[:,0]
    # # print(specific_weights)
    # # specific_covariance = c[:4,:4]
    # # print(specific_covariance)
    # # variance = calc_variance(specific_weights, specific_covariance)
    # # print(variance)
    #
    # # calculate multi-period portfolio variance
    # port_variance = calc_multi_variance(port_weights, c, total_periods, asset_num)
    # print(port_variance)
    #
    # # load return data
    # df = pd.read_csv('SinglePeriod_20X12.csv', header=None)
    # r = np.array(df)
    # # calculate multi-period portfolio return
    # port_return = calc_multi_return(port_weights, r, total_periods, asset_num)
    # print(port_return)
